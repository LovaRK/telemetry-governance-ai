...
export async function GET(
  ...
  try {
    ...
    const events = await telemetryService.getAuditHistory(indexName, startTime, endTime);

    // Calculate trust score progression (extract state transitions and confidence changes)
    const progression = events
      .filter(
        (e) =>
          e.event_type === 'GOVERNANCE_STATE_TRANSITION' ||
          e.event_source === 'snapshot'
      )
      .map((e) => ({
    ...
    // Count mutations and errors
    const mutations = {
      total: events.length,
      successful: events.filter((e) => e.event_type === 'GOVERNANCE_MUTATION_SUCCESS').length,
      failed: events.filter((e) => e.api_response_code && e.api_response_code >= 400).length,
      abandoned: events.filter((e) => e.event_type === 'GOVERNANCE_MUTATION_ABANDONED').length,
    };

    const errors = {
      versionCollisions: events.filter((e) => e.event_type === 'GOVERNANCE_VERSION_COLLISION').length,
      invalidationFailures: events.filter((e) => e.event_type === 'GOVERNANCE_CACHE_DESYNC').length,
      forbiddenTransitions: events.filter((e) => e.event_type === 'GOVERNANCE_FORBIDDEN_TRANSITION').length,
      rateLimited: events.filter((e) => e.event_type === 'GOVERNANCE_RATE_LIMITED').length,
    };
