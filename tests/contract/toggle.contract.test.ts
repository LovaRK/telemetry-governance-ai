...
describe('Contract: explainability toggle state', () => {
  test('enabled=false and enabled=true are both persisted safely', async () => {
    ...
    const userId = 'toggle-contract-user';

    const setFalse = await authPost('/api/settings/explainability', token, { explainabilityMode: false }, undefined, userId);
    expect(setFalse.status).toBe(200);
    const getFalse = await authGet('/api/settings/explainability', token, undefined, userId);
    ...
    const setTrue = await authPost('/api/settings/explainability', token, { explainabilityMode: true }, undefined, userId);
    expect(setTrue.status).toBe(200);
    const getTrue = await authGet('/api/settings/explainability', token, undefined, userId);
